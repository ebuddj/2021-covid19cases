...
content = urlopen(req)
df = pd.read_csv(content, usecols=['continent','location','date','new_cases_smoothed_per_million'])
...
for country in df.location.unique():
  ...
  for index, values in (df[df['location'] == country]).iterrows():
    ...
    if values.new_cases_smoothed_per_million > 0:
      ...
      data[country_data_name][values.date] = values.new_cases_smoothed_per_million
...
for erno_country in erno_countries:
  ...
  for index, values in (df[df['location'] == erno_country]).iterrows():
    if values.new_cases_smoothed_per_million != 0:
      previous_value = values.new_cases_smoothed_per_million
      data[erno_country][values.date] = values.new_cases_smoothed_per_million
    else:
